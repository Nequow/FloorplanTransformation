...
def np_print(arrays):
    for name, array in arrays.items():
        print(
            "*** ", name, array.shape, (array.min(), array.max()), np.unique(array).size
        )
...
def main(img_path):
    ...
    model.load_state_dict(
        torch.load(
            "checkpoint/floorplan/checkpoint.pth", map_location=torch.device("cpu")
        )
    )
    ...
    icon_heatmaps = (
        torch.nn.functional.softmax(icon_pred[0], dim=-1).detach().cpu().numpy()
    )
    room_heatmaps = (
        torch.nn.functional.softmax(room_pred[0], dim=-1).detach().cpu().numpy()
    )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        img_path = sys.argv[1].split("--im_path=")[1]
    else:
        img_path = "../../../Floor plan datasddet/10.png"
    main(img_path)
